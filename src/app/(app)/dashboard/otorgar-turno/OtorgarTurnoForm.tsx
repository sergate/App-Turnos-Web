...
import { otorgarTurno } from "./actions";
...
export default function OtorgarTurnoForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await otorgarTurno(fd);
      if (!res.success) {
        setError(res.error ?? "No se pudo otorgar el turno.");
      ...
      setExito(`Turno otorgado a "${providerName.trim()}" para ${formatFecha(slotSeleccionado.slotDate)}.`);
      setProviderName("");
      setProviderEmail("");
      setDetalle("");
      setFoto(null);
  ...
  return (
    <div className="max-w-xl mx-auto">
      <h1 className="text-lg font-bold text-slate-800 mb-1">Otorgar turno</h1>
      <p className="text-sm text-slate-500 mb-6">
        Cargá un turno de entrega directamente (por ejemplo, si el proveedor coordinó por teléfono). Queda
      ...
      {!cargandoSlots && fechasDisponibles.length > 0 && (
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          >
            {enviando ? "Guardando..." : "Otorgar turno"}
          </button>
        </form>
      )}
    </div>
