...
export async function otorgarTurno(formData: FormData): Promise<ActionResult> {
  ...
  const { error: insertError } = await supabase.from("turnos").insert({
    ...
    detalle: detalle || "(cargado por el equipo, sin detalle)",
  ...
  const fecha = formatFecha(slotDate);
  const horario = `${formatHora(slotStartTime)} - ${formatHora(slotEndTime)}`;

  if (providerEmail) {
    await sendNotificationEmail({
      to: providerEmail,
      subject: "Turno de entrega confirmado",
      html: `
        <p>Hola ${providerName},</p>
        <p>Te confirmamos un turno de entrega:</p>
        <ul>
  ...
  await supabase.from("activity_log").insert({
    ...
    action: "turno_otorgado",
    detalle: `Otorgó un turno a "${providerName}" para el ${fecha} (${horario}).`,
  });
